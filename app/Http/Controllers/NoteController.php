...
class NoteController extends Controller
{
    ...
    function saveTxt(Request $request)
    {
        ...
        $content_file = $request->input("content_file");
        $mime = $request->input("mime");

        if ($noteId == 0) {
            $note = Note::create(["id" => 0,
                "folder_id" => $folder_id,
                "filename" => $filename,
            ...
            $note->setAttribute("folder_id", $request->input("folder_id"));
            $note->setAttribute("filename", $request->input("filename"));
            $note->setAttribute("username", Auth::user()->email);
            $note->setAttribute("content_file", $request->input("content_file"));
            $note->setAttribute("mime", $request->input("mime"));

            $note->folder_id = $request->input("folder_id");
            $note->filename = $request->input("filename");
            $note->username = Auth::user()->email;
            $note->content_file = $request->input("content_file");
            $note->mime = $request->input("mime");

            $note->fillable(["id" => $noteId,
        ...
        //return Redirect::to('note/edit/' . $note->getAttribute('id'))->with(["Message" => "Sauvegardé"]);
    }
    ...
    function saveVideoFromWebcam(Request $request)
    {
        $text = "<h1>Result</h1>";
